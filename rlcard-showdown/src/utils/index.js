...
export const spadesSuits = ['S', 'H', 'D', 'C'];
export const spadesRanks = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A'];
export const spadesDeck = spadesSuits.flatMap((suit) => spadesRanks.map((rank) => `${suit}${rank}`));
export const spadesDeckIndex = spadesDeck.reduce((acc, card, idx) => {
    acc[card] = idx;
    return acc;
}, {});
...
export function spadesActionIdToLabel(actionId) {
    ...
    if (actionId === 53) return 'blind_nil';
    if (actionId === 54) return 'nil';
    if (actionId >= 55 && actionId <= 67) return `bid_${actionId - 54}`;
    return null;
}

export function spadesCardToActionId(card) {
    return spadesDeckIndex[card];
}
