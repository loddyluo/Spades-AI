#!/usr/bin/env python3
"""stats_matchup.py: 统计 log 文件中 hp_A vs hp_A 的得墩/叫牌分布。
...
def parse_log(path: str) -> dict:
    ...
    返回:
        our_contracts: list of (sum_bid, sum_tricks) for hp_A team per game
        go_contracts:  list of (sum_bid, sum_tricks) for hp_A team per game
        our_nils:      list of tricks_won for nil contracts (x/0) by hp_A
        go_nils:       list of tricks_won for nil contracts (x/0) by hp_A
    """
    ...
    if seat_specs_line:
        specs = eval(seat_specs_line.group(1))  # ['hp_A', 'hp_A', ...]
    else:
        specs = ['hp_A', 'hp_B', 'hp_A', 'hp_B']

    # Determine which seats belong to hp_A in game 0
    base_our_seats = {i for i, s in enumerate(specs) if s == "hp_A"}
    base_go_seats = {i for i, s in enumerate(specs) if s == "hp_B"}
...
def plot_results(
    ...
    """绘制热力图、nil 分布图、总叫墩直方图和得墩-叫墩差分布图."""
    ...
    ax.set_title("hp_A (bid_sum x tricks_sum)", fontsize=13)
    ...
    ax.set_title("hp_B (bid_sum x tricks_sum)", fontsize=13)
    ax.set_xlabel("tricks_sum")
    ax.set_ylabel("bid_sum")
    sum_cheng = 0
    for b in range(max_bid):
    ...
    if our_nil_tricks:
        ...
        ax.set_title(f"hp_A nil tricks (n={len(our_nil_tricks)})", fontsize=13)
        ax.set_xlabel("tricks")
        ax.set_ylabel("count")
    else:
        ax.text(0.5, 0.5, "no nil contracts", ha="center", va="center", transform=ax.transAxes)
        ax.set_title("hp_A nil", fontsize=13)
    ...
    if go_nil_tricks:
        ...
        for count, patch in zip(counts, patches):
            if count > 0:
                ax.text(patch.get_x() + patch.get_width() / 2, patch.get_height(),
                        str(int(count)), ha="center", va="bottom", fontsize=10)
        ax.set_title(f"hp_B nil tricks (n={len(go_nil_tricks)})", fontsize=13)
        ax.set_xlabel("tricks")
        ax.set_ylabel("count")
    else:
        ax.text(0.5, 0.5, "no nil contracts", ha="center", va="center", transform=ax.transAxes)
        ax.set_title("hp_B nil", fontsize=13)
    ...
    for ax, contracts, team in [(ax_our_diff, our_contracts, "hp_A"),
                                 (ax_go_diff, go_contracts, "hp_B")]:
        diffs = [t - b for b, t in contracts]
        min_diff = min(diffs)
        max_diff = max(diffs)
        bins = np.arange(min_diff - 0.5, max_diff + 1.5, 1)
        counts, bins_edges, patches = ax.hist(diffs, bins=bins, alpha=0.7, edgecolor="black")
        for count, patch in zip(counts, patches):
            if count > 0:
                ax.text(patch.get_x() + patch.get_width() / 2, patch.get_height(),
                        str(int(count)), ha="center", va="bottom", fontsize=9)
        ax.set_title(f"{team} tricks_sum - bid_sum (n={len(diffs)})", fontsize=13)
        ax.set_xlabel("tricks_sum - bid_sum")
        ax.set_ylabel("count")
        ax.set_xticks(range(min_diff, max_diff + 1))
...
def plot_subset(
    ...
    for ax, arr, team in [(ax_our, our_arr, "hp_A"), (ax_go, go_arr, "hp_B")]:
    ...
    for ax, nils, team in [(ax_our_nil, our_nils, "hp_A"),
                           (ax_go_nil, go_nils, "hp_B")]:
        if nils:
...
def main() -> None:
    ...
    print(f"\n总游戏数: {A}")
    print(f"hp_A 队伍记录数: {our_total}  (应为 {A})")
    print(f"hp_A 队伍记录数: {go_total}  (应为 {A})")

    print_2d_array(our_arr, "hp_A", our_total)
    print_2d_array(go_arr, "hp_B", go_total)

    print_nil_dist(data["our_nils"], "hp_A")
    print_nil_dist(data["go_nils"], "hp_B")
    ...
    for tb in [10, 11, 12]:
        ...
        print_2d_array(our_arr_tb, f"hp_A (总叫{tb})", our_arr_tb.sum())
        print_2d_array(go_arr_tb, f"hp_A (总叫{tb})", go_arr_tb.sum())
