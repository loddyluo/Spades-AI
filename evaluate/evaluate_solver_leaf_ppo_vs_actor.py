"""Paired evaluation of one solver-leaf actor versus a frozen actor opponent."""
...
from rl.solver_leaf_env import OpponentPoolConfig, ProductionDuplicateCollector
from rl.solver_leaf_ppo import load_exported_actor


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one 536-d PPO actor against a frozen PPO actor",
    ...
    parser.add_argument("--seed", type=int, default=536_700)
    parser.add_argument("--base-shuffle-seed", type=int, default=253_600_000)
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _, opponent_metadata = load_exported_actor(
        Path(args.opponent_checkpoint), device="cpu"
    )
    for label, item in (("candidate", metadata), ("opponent", opponent_metadata)):
        if item.get("residual_checkpoint_sha256") != DEPLOYED_CHECKPOINT_SHA256:
            raise ValueError(f"{label} actor used a different Residual bidder")
    if metadata["hidden_dims"] != opponent_metadata["hidden_dims"]:
        raise ValueError("candidate and opponent actor architectures differ")

    opponent_pool = OpponentPoolConfig(
        rule_weight=0.0,
        champion_weight=1.0,
        champion_checkpoint=str(Path(args.opponent_checkpoint)),
    )
    with ProductionDuplicateCollector(
        workers=args.workers,
        actor_hidden_dims=metadata["hidden_dims"],
    ...
    report: dict[str, Any] = {
        "schema": "solver-leaf-ppo-vs-actor-evaluation-v1",
        "comparison": "candidate-ppo-vs-frozen-ppo",
