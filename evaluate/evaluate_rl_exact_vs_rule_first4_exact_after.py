"""Evaluate rl_exact vs RuleExactFirst4Player in Spades matchups.

The rl_exact player uses a policy network (MLP) for the first 4 tricks
(16 cards) and the exact double-dummy solver with IS-weighted determinization
for the remaining cards.

The RuleExactFirst4Player uses rule-based play for the first 4 tricks
and the same exact double-dummy solver with IS-weighted determinization
for the remaining cards.

Both use their respective sampling of opponent hands and exact solving
...
# ────────────────────────────────────────────────────────────────────────
# BothPlayer — selects RL checkpoint based on nil bids
# ────────────────────────────────────────────────────────────────────────
class BothPlayer(RLExactPlayer):
    ...
    - Someone bids nil/blind_nil → use nil-specific policy
    - No one bids nil           → use non-nil policy
    """
...
def _patched_init_worker(args) -> None:
    """Initialize worker: call original, then load RL policy networks and exact solver."""
...
def _patched_build_players(args, runtime, game_seed, seat_specs=None):
    """Extended build_players with rl_exact and rule_first4_exact support."""
    global _worker_rl_policy_nonil, _worker_rl_policy_nil
    global _worker_exact_solver, _worker_encoder
    ...
    # If neither rl_exact nor rule_first4_exact, fall back to original
    if not has_rl and not has_rule_first4:
    ...
    for seat_index, spec in enumerate(seat_specs_resolved):
        if spec == "rule_first4_exact":
            ...
            # 传入 nil 策略网络，使 rule_first4_exact 在有人叫 nil 时
            # 也能像 rl_exact 一样用 55_2nil.pt 处理前 4 墩
            p_nil = _worker_rl_policy_nil
            encoder = _worker_encoder

            # Load hyperparameter config from YAML if specified
            from strategy.hyperparam_config import HyperparamConfig
            rule_config = None
            config_path = getattr(args, 'rule_config', None)
            if config_path:
                rule_config = HyperparamConfig.from_yaml(config_path)
            ...
            player = RuleExactFirst4Player(
                ...
                policy_net_nil=p_nil,
                encoder=encoder,
                hyperparam_config=rule_config,
            )
...
# ── CLI ────────────────────────────────────────────────────────────────
def parse_args():
    """Parse args with both rl_exact and rule_first4_exact support."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Evaluate rl_exact vs rule_first4_exact in Spades.",
    ...
    # MCTS-related (for our_mcts player, kept for compatibility)
    ...
    # Rule-based hyperparameter config
    parser.add_argument("--rule-config", type=str, default="./configs/8.yaml",
                        help="Path to YAML config for RuleExactFirst4Player hyperparameters")
...
def main() -> None:
    ...
    args = parse_args()
    print(f"=== rl_exact vs rule_first4_exact Evaluation ===")
    ...
    print(f"Exact threshold: {args.our_exact_threshold} "
          f"(first {52 - args.our_exact_threshold} cards use RL/rule-based)")
    print(f"Games: {args.num_games}, Seed: {args.seed}, "
          f"Symmetric: {args.symmetric_seat_swap}")
    print(f"Workers: {args.num_workers}")
    if args.rule_config:
        print(f"Rule config: {args.rule_config}")
    print()

    # ── For single-process mode, load solver and policies here (fork inherits them) ──
