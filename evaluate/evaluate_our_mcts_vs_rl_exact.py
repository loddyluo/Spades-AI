"""Evaluate our_mcts vs rl_exact in Spades matchups.
...
Usage:
    python evaluate/evaluate_our_mcts_vs_rl_exact.py \
        --p0 our_mcts --p1 rl_exact --p2 our_mcts --p3 rl_exact \
        --num-games 100 --seed 8880000 --num-workers 20

Defaults: p0=our_mcts, p1=rl_exact, p2=our_mcts, p3=rl_exact
"""
...
# Rule-based v2 import (kept for flexibility)
...
def _patched_build_players(args, runtime, game_seed, seat_specs=None):
    """Extended build_players with RL-Exact and DDS player support."""
    ...
    # If no rl_exact or dds in specs, fall back to original (handles our_mcts etc.)
...
# ── CLI ────────────────────────────────────────────────────────────────
def parse_args():
    """Parse args with both our_mcts and rl_exact support."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Evaluate our_mcts vs rl_exact in Spades.",
...
def main() -> None:
    ...
    args = parse_args()
    print(f"=== our_mcts vs rl_exact Evaluation ===")
    ...
    print(f"Exact threshold: {args.our_exact_threshold} "
          f"(first {52 - args.our_exact_threshold} cards use RL/MCTS)")
