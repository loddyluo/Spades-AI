"""Evaluate RL-Exact vs Rule-Based V2 in Spades matchups.
...
The Rule-Based V2 player uses a handcrafted rule-based strategy with
MLP bidding.

Checkpoint selection for RL depends on nil bids:
- Someone bids nil → 55_2nil.pt
- No one bids nil → 55_2.pt

Usage:
    python evaluate/evaluate_rl_vs_rule_v2.py \
        --p0 rl_exact --p1 go_rule_2 --p2 rl_exact --p3 go_rule_2 \
        --num-games 64 --seed 42 --num-workers 32
"""
...
# RL imports (same as evaluate_dds_vs_rl.py)
...
# ────────────────────────────────────────────────────────────────────────
# BothPlayer — selects checkpoint based on nil bids (from evaluate_dds_vs_rl.py)
# ────────────────────────────────────────────────────────────────────────
...
def _patched_build_players(args, runtime, game_seed, seat_specs=None):
    """Extended build_players with RL-Exact player support."""
    ...
    for seat_index, spec in enumerate(seat_specs_resolved):
        ...
        # Fallback to original for other specs (go_rule_2, our_mcts, etc.)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate RL-Exact vs Rule-Based V2 in Spades.",
    ...
    parser.add_argument("--p1", type=str, default="go_rule_2",
                        help="Seat 1 model spec")
    parser.add_argument("--p2", type=str, default="rl_exact",
                        help="Seat 2 model spec")
    parser.add_argument("--p3", type=str, default="go_rule_2",
...
def main() -> None:
    ...
    args = parse_args()
    print(f"=== RL-Exact vs Rule-Based V2 Evaluation ===")
