  ...
  MoveType const * mply)
{
  ...
  // SPADES EXTENSION: save and update spades_broken
  posPoint->spades_broken_saved[depth] = posPoint->spades_broken;
  if (s == 0) posPoint->spades_broken = 1;
  ...
  const std::shared_ptr<ThreadData>& thrp)
{
  ...
  // SPADES EXTENSION: restore spades_broken
  posPoint->spades_broken = posPoint->spades_broken_saved[depth];
