  ...
  FutureTricks * futp) -> int
{
  ...
  // SPADES EXTENSION: initialize spades_broken from deal
  thrp->lookAheadPos.spades_broken = dl.spades_broken;
  // Also check: if any spade appears in currentTrickSuit, spades are broken
  for (int i = 0; i < 3; i++) {
    if (dl.currentTrickRank[i] != 0 && dl.currentTrickSuit[i] == 0) {
      thrp->lookAheadPos.spades_broken = 1;
    }
  }
