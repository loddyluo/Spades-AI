...
auto Moves::MoveGen0(const int tricks, const Pos &tpos,
  ...
  for (suit = 0; suit < DDS_SUITS; suit++) {
    // SPADES EXTENSION: skip spades (suit==0) when not broken,
    // trump is spades, and leader has cards in other suits.
    if (suit == 0 && tpos.spades_broken == 0 && trump == 0) {
      // Check if leader has any non-spade cards
      bool has_non_spades = false;
      for (int s = 1; s < DDS_SUITS; s++) {
        if (tpos.rank_in_suit[leadHand][s] != 0) {
          has_non_spades = true;
          break;
        }
      }
      if (has_non_spades)
        continue;  // skip spades entirely
    }
