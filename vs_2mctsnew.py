#!/usr/bin/env python3
"""vs_2mctsnew.py: 双 MCTS 玩家 vs 人类玩家的黑桃王交互程序。

两个 MCTS 玩家在同一队（要么 seats 0&2 = team 0，要么 seats 1&3 = team 1）。
两个 MCTS 玩家彼此看不到对方手牌，各自独立决策（各自使用自己的 TruncatedMCTSStrategy）。

与 vs_2mcts.py 不同之处:
  - 叫牌使用 bid_nsfp.pt MLP 模型，而非 hand_strength 启发式
  - MCTS 搜索内层展开时使用 RuleBasedPlayer 作为 prior oracle
...
交互格式:
    ...
    Then 叫牌阶段 (座位顺序 0→1→2→3):
      - MCTS 玩家: 程序用 bid_nsfp.pt 计算并输出叫牌
      - 人类玩家: 你输入他们的叫牌
...
def main() -> None:
    ...
    # ── Load bid model ──
    bid_mlp = load_bid_mlp_model(args.bid_checkpoint, "cpu")
    bid_player = MLPBidPlayer(bid_mlp, "cpu")
    ...
    original_mcts_hands = {
        mcts_seats[0]: list(hand_a),
        mcts_seats[1]: list(hand_b),
    }
    ...
    for p in range(4):
        if p in mcts_seats:
            # Use MLP bid model; state already has previous bids from earlier players
            state.turn = p
            ...
            if bid_type_name == "NIL":
                ...
                if bid_val == 0:
                    bid_val = 1  # MLP may output 0 for normal; clamp to 1
        ...
        # Update state immediately so subsequent players see this bid
    ...
    def refill_for_observer(obs: int) -> None:
        ...
        Only the observer's (*obs*) hand is preserved.  All other hands
        (including the other MCTS partner) get random cards from the unseen
        pool, sized to match how many cards each player still holds.
        The IS determinization inside the strategy will resample them from
        scratch, so the exact content here is irrelevant.
        """
        # Reconstruct MCTS hands from original dealt cards minus played cards
        # so the observer always gets their real remaining hand.
        for seat in mcts_seats:
            ...
            for pid, c in state.table_cards:
                if pid == seat:
                    played_ids.add(c.card_id)
            state.hands[seat] = [
                c for c in original_mcts_hands[seat] if c.card_id not in played_ids
            ]

        preserved = {obs}  # Only the current observer's hand stays intact
        used: set[int] = set()
        for p in preserved:
        ...
        for p in range(4):
            ...
            i += n
        # Keep hand_bitsets in sync — C++ exact solver relies on them
        state.hand_bitsets = [
            sum(1 << c.card_id for c in state.hands[p]) for p in range(4)
        ]
    ...
    for _ in range(13):
        ...
        for pos_in_trick in range(4):
            ...
            if cur in mcts_seats:
                ...
                state.turn = cur  # ensure strategy sees the correct current player
