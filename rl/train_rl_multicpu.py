...
def worker_batch(args_tuple: tuple) -> list[dict]:
    ...
    # 每个 worker 独立创建自己的资源
    policy_net = PolicyMLP(input_dim=1229, hidden_dims=hidden_dims)
...
def train(args: argparse.Namespace) -> None:
    ...
    writer = None
    if args.tensorboard:
        log_path = Path(args.log_dir) / f"05311500_seed{args.seed}_lr{args.lr}_hid{'_'.join(str(h) for h in args.hidden_dims)}_w{args.num_workers}"
    ...
    with ctx.Pool(args.num_workers) as pool:
        for batch_start in range(0, num_episodes, args.update_interval):
            ...
            if batch_end % args.save_interval == 0:
                cp_path = save_dir / f"policy_ep{batch_end}.pt"
