"""
预训练 RL policy gradient 脚本（多核版本）：单模型 55 维输出。

模型输出 55 维 logits:
- 前 52 维: 领出时使用，对应 52 张牌的 card_id
- 后 3 维: 跟牌时使用，对应 3 个策略选项:
    0) 出最小牌
    1) 出能赢当前墩的最小牌
    2) 出最大牌
...
MODEL_OUTPUT_DIM = 55  # 52(领出: card_id) + 3(跟牌选项)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Pretrain RL policy gradient (multi-CPU, 55-dim): 领出52+跟牌3选项",
    ...
    parser.add_argument("--save-dir", type=str, default="rl_checkpoints/pretrain_170dim",
                        help="模型保存目录")
    ...
    parser.add_argument("--log-dir", type=str, default="runs/pretrain_rl_170dim",
                        help="TensorBoard 日志目录")
    parser.add_argument("--load-checkpoint", type=str, default=None,
                        help="从指定路径加载 checkpoint（.pt 文件），在此基础上继续训练")
...
def worker_batch(args_tuple: tuple) -> list[dict]:
    ...
    每个 trajectory entry 包含 action_logit_idx 和 legal_logit_indices，
    用于主进程按 55 维 output 做梯度更新。
    """
    ...
    # 创建单模型（55 维输出）
    policy_net = PolicyMLP(input_dim=170, hidden_dims=hidden_dims, output_dim=MODEL_OUTPUT_DIM)
...
def train(args: argparse.Namespace) -> None:
    ...
    # ── 主进程：创建单策略网络（55 维输出） ───────────────────────
    policy_net = PolicyMLP(input_dim=170, hidden_dims=args.hidden_dims,
    ...
    if args.load_checkpoint:
        ...
        if cp_path.exists():
            ...
            # 加载后切回训练模式
            policy_net.train()
            print(f"从 checkpoint 加载模型: {cp_path.resolve()}", flush=True)
        else:
            print(f"警告: checkpoint 不存在: {cp_path}，将使用随机初始化", flush=True)
    else:
        print("未指定 --load-checkpoint，使用随机初始化", flush=True)
    ...
    print("=" * 72, flush=True)
    print("Pretrain RL Policy Gradient (MULTI-CPU, 55-dim 单模型): 领出52+跟牌3选项", flush=True)
    print(f"总对局数: {args.num_games} ({num_episodes} episodes × 2 games)", flush=True)
    print(f"模型输出维度: {MODEL_OUTPUT_DIM} (52 领出 + 3 跟牌)", flush=True)
    ...
    writer = None
    if args.tensorboard:
        log_path = Path(args.log_dir) / f"20900_55dim_seed{args.seed}_lr{args.lr}_hid{'_'.join(str(h) for h in args.hidden_dims)}_w{args.num_workers}"
    ...
    with ctx.Pool(args.num_workers) as pool:
        for batch_start in range(0, num_episodes, args.update_interval):
            ...
            for worker_results in results:
                for ep_res in worker_results:
                    for traj in ep_res["trajectories"]:
                        ...
                        all_rewards.append(traj["reward_val"])

            # ── 梯度更新 ──────────────────────────────────────────
            if accumulated_trajs:
                n_trajs = len(accumulated_trajs)

                # 计算基线（batch 内所有轨迹的均值）
                rewards_batch = [t["reward_val"] for t in accumulated_trajs]
                baseline = float(np.mean(rewards_batch))

                has_entropy = "entropy_val" in accumulated_trajs[0]
                ...
                for epoch in range(args.num_epochs):
                    ...
                    for traj in accumulated_trajs:
                        feature = traj["feature"]
                        feat_tensor = torch.from_numpy(feature).float().unsqueeze(0)
                        logits = policy_net(feat_tensor).squeeze(0)  # 55 维

                        # 55 维 mask：只允许合法 logit 索引
                        legal_indices = traj["legal_logit_indices"]
                        mask = torch.full((MODEL_OUTPUT_DIM,), float("-inf"))
                        for idx in legal_indices:
                            mask[idx] = 0.0
                        masked_logits = logits + mask
                        log_probs = torch.log_softmax(masked_logits, dim=0)
                        log_prob = log_probs[traj["action_logit_idx"]]
                        ...
                        # 塌缩监控
                        with torch.no_grad():
                            n_legal = max(len(legal_indices), 1)
                    ...
                    if writer is not None:
                        ...
                        writer.add_scalar("train/reinforce_loss", last_reinforce_loss, tb_step)
                        writer.add_scalar("train/grad_norm", last_grad_norm, tb_step)
                        writer.add_scalar("train/baseline", baseline, tb_step)
                        writer.add_scalar("train/mean_max_prob", last_mean_max_prob, tb_step)
                        writer.add_scalar("train/mean_entropy", last_mean_entropy, tb_step)
                        writer.add_scalar("train/mean_norm_entropy", last_mean_norm_entropy, tb_step)

                # 打印日志
                print(f"  [Update] ep {batch_start+1}-{batch_end} (×{args.num_epochs} epochs, "
                      f"trajs={n_trajs}), "
                      f"loss={last_reinforce_loss:.3f}, gn={last_grad_norm:.3f}, bl={baseline:.1f}, "
                      f"avg_rew={np.mean(rewards_batch):+.2f}, "
                      f"pos_adv={last_n_pos_adv}/{n_trajs}, "
                      f"max_p={last_mean_max_prob:.3f}, H={last_mean_entropy:.3f}", flush=True)
            ...
            # ── 日志 ──────────────────────────────────────────────────
            if batch_end % 40 == 0:
                recent = all_rewards[-40:] if len(all_rewards) >= 40 else all_rewards
                avg_reward = np.mean(recent) if recent else 0.0
            ...
            # ── TensorBoard 全局指标 ────────────────────────────────
            if writer is not None and batch_end % 300 == 0:
                recent_300 = all_rewards[-300:] if len(all_rewards) >= 300 else all_rewards
                if recent_300:
                    writer.add_scalar("train/avg_reward", np.mean(recent_300), batch_end)

            # ── 保存 checkpoint ────────────────────────────────────
            if batch_end % args.save_interval == 0:
                cp_path = save_dir / f"pretrain_policy_ep{batch_end}.pt"
                policy_net.save(str(cp_path))
                print(f"  -> 已保存: {cp_path}", flush=True)
    ...
    print(flush=True)
    print("=" * 72, flush=True)
    print("预训练完成！（单模型 55 维输出：领出52 + 跟牌3选项）", flush=True)
    print(f"总耗时: {t_elapsed:.0f}s", flush=True)
    print(f"最终模型: {final_path}", flush=True)
    print(f"全局平均逐牌奖励: {avg_reward_all:+7.2f}")
    print("=" * 72, flush=True)
