...
牌局状态：
- 6 墩已完成（24 张牌已打出），第 7 墩正在进行
- 玩家 0 首攻 ♣2，玩家 1 跟 ♣3，玩家 2 跟 ♣4
- 桌上 3 张牌，轮到玩家 3 出牌
- 手牌共 25 张：玩家 0/1/2 各 6 张，玩家 3 有 7 张

设计意图：
- 玩家 3 无梅花（首攻为梅花，玩家 3 无同花色可跟），因此有 7 个合法动作
- 牌数适中，精确求解器仍可处理，MCTS 也能展开搜索
"""
...
def create_25card_state():
    ...
    # ---- 25 张手牌（不包含已打到桌上的牌） ----
    # 玩家 0（队伍0）：♠A ♠K ♠Q ♥A ♣A ♣Q，已打出 ♣2
    # 玩家 1（队伍1）：♥K ♥Q ♦A ♦K ♦Q ♣7，已打出 ♣3
    # 玩家 2（队伍0）：♠3 ♠4 ♦3 ♥4 ♥5 ♣8，已打出 ♣4
    # 玩家 3（队伍1）：♠5 ♠6 ♥6 ♥7 ♦4 ♦5 ♦6（无梅花！可出任意牌）
    hand_strs = [
        ["SA", "SK", "SQ", "HA", "CA", "CQ"],          # P0 队伍0 6 张
        ["HK", "HQ", "DA", "DK", "DQ", "C7"],          # P1 队伍1 6 张
        ["S3", "S4", "D3", "H4", "H5", "C8"],          # P2 队伍0 6 张
        ["S5", "S6", "H6", "H7", "D4", "D5", "D6"],   # P3 队伍1 7 张（无梅花）
    ]
    ...
    # 赢墩：P0=2 P1=2 P2=1 P3=1 → 队伍0=3 队伍1=3
    state.tricks_won = [2, 2, 1, 1]
...
def test_mcts_solver(state):
    ...
    solver = DoubleDummySolver(max_iterations=3000, exploration_weight=1.4)
...
def main():
    print("创建测试牌局（25 张手牌，6 墩已完成，第 7 墩进行中）...\n")

    state = create_25card_state()
    ...
    print("\n--- 简要分析 ---")
    print(f"当前桌面 ♣2/♣3/♣4，玩家3 无梅花可垫任意花色")
    print(f"♣A 和 ♣Q 仍在玩家0手中，♣7 和 ♣8 分别在玩家1/2手中")
