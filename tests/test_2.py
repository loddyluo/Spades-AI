...
def create_two_trick_state():
    ...
    # 分配手牌：每玩家2张
    hands = [
        [target_cards[0], target_cards[1]],  # P0: ♠A, ♠Q
        [target_cards[2], target_cards[3]],  # P1: ♥2, ♥3
        [target_cards[4], target_cards[5]],  # P2: ♥4, ♥5
        [target_cards[6], target_cards[7]],  # P3: ♠K, ♣A
    ]
    ...
    state.bids = [
        ...
        Bid(player_id=3, value='bid_4'),  # 02队：60分，13队：-80分
    ]
...
def manual_calculation():
    ...
    玩家0持有♠A和♠Q，玩家3持有♠K和♣A。
    P0首攻，无论如何P0都能赢得全部两墩。

    第12墩方案一：P0出♠A → P3跟♠K → P0赢
    第13墩方案一：P0出♠Q → 无人有黑桃 → P0赢

    第12墩方案二：P0出♠Q → P3出♠K赢
    第13墩方案二：P3出♣A → P0无草花，♠A将吃 → P0赢

    最终赢墩数：玩家0 = 2, 其他人 = 0

    结算：
    队伍0：玩家0（叫2得2，完成，+20）+ 玩家2（nil得0，成功，+50）= 70
    队伍1：玩家1（叫1得0，未完成，-10）+ 玩家3（叫1得0，未完成，-10）= -20
    得分差（队伍0 - 队伍1）: 70 - (-20) = 90
    """
    return 140.0
